...
class overfitClassifier_DN(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = self.dropout2(x)
        x = self.active1(self.layer2(x))
        x = self.BN2(x)

        x = self.dropout3(x)
        x = self.active1(self.layer3(x))
        x = self.BN3(x)

        x = self.dropout4(x)
        x = self.active1(self.layer4(x))
        x = self.BN4(x)

        x = self.dropout5(x)
        x = self.active1(self.layer5(x))
        x = self.BN5(x)
...
overfitModel_dropout_Normalization = overfitClassifier_DN()
train_risk_overfitted_dropout_Normalization, test_risk_overfitted_dropout_Normalization = training_loop(overfitModel_dropout_Normalization)

# complete <plot training risk>
# complete <plot test risk>
# Plot training risk

plt.plot(train_risk_overfitted_dropout_Normalization, label='Training Risk')

# Plot test risk
plt.plot(test_risk_overfitted_dropout_Normalization, label='Test Risk')

# Add title and labels to the plot
plt.title('Training and Test Risk over Epochs with Dropout and Batch Normalization')
plt.xlabel('Epochs')
plt.ylabel('Risk')
